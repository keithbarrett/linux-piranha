...
#define LVS_SAW_PRIMARYSERVER       (1 << 0)
#define LVS_SAW_BACKUPSERVER        (1 << 1)
#define LVS_SAW_BACKUPMODE          (1 << 2)
#define LVS_SAW_USEHEARTBEAT        (1 << 3)
#define LVS_SAW_KEEPALIVETIME       (1 << 4)
#define LVS_SAW_DEADTIME            (1 << 5)
#define LVS_SAW_USEFILESYNC         (1 << 6)
#define LVS_SAW_RSHCOMMAND          (1 << 7)
#define LVS_SAW_VSADM               (1 << 8)
#define LVS_SAW_REDIRECTTYPE        (1 << 9)
#define LVS_SAW_HEARTBEATPORT       (1 << 10)
#define LVS_SAW_NATROUTER           (1 << 11)
#define LVS_SAW_NAT_NMASK	        (1 << 12)
#define LVS_SAW_BACKUPACTIVE        (1 << 13)
#define LVS_SAW_VIRTUALSERVICE      (1 << 14)
#define LVS_SAW_FAILOVERSERVICE     (1 << 15)
#define LVS_SAW_SERVICETYPE         (1 << 16)
#define LVS_SAW_DEBUG_LEVEL         (1 << 17)

#define LVS_SAW_VS_CLIENTMONITOR    (1 << 1)
#define LVS_SAW_VS_VIRTUALADDRESS   (1 << 2)
#define LVS_SAW_VS_PROTOCOL         (1 << 3)
#define LVS_SAW_VS_SCHEDULER        (1 << 4)
#define LVS_SAW_VS_ISACTIVE         (1 << 5)
#define LVS_SAW_VS_PORT             (1 << 6)
#define LVS_SAW_VS_LOADMONITOR      (1 << 7)
#define LVS_SAW_VS_TIMEOUT          (1 << 8)
#define LVS_SAW_VS_REENTRYTIME      (1 << 9)
#define LVS_SAW_VS_MASTERSERVER     (1 << 10)
#define LVS_SAW_VS_DIRS             (1 << 11)
#define LVS_SAW_VS_PERSISTENT       (1 << 12)
#define LVS_SAW_VS_PMASK            (1 << 13)
#define LVS_SAW_VS_SEND             (1 << 14)
#define LVS_SAW_VS_EXPECT           (1 << 15)
#define LVS_SAW_VS_FS_START         (1 << 17)
#define LVS_SAW_VS_FS_STOP          (1 << 18)
#define LVS_SAW_VS_VIP_NMASK        (1 << 19)
 
#define LVS_SAW_AS_ADDRESS          (1 << 0)
#define LVS_SAW_AS_ISACTIVE         (1 << 1)
#define LVS_SAW_AS_PORT             (1 << 2)
#define LVS_SAW_AS_WEIGHT           (1 << 3)
#define LVS_SAW_AS_LOGFILE          (1 << 4)
...
#define TOKEN(a) (strcmp(toks.argv[0], a) == 0)
...
void freeTokens(struct tokenList * toks) {
    toks->preSpace      = 0;
    toks->preSpaceLen   = 0;
    toks->argv[0]       = 0;
    toks->quoteType[0]  = 0;
...
int tokenizeString( char * string, struct tokenList * tokens) {
...
static void appendToOutput(struct outputBuffer * ob, char * text,
                           int len) {

    if (!ob || ob->block) return;
...
static void outputFormat(struct outputBuffer * ob, char * format, ...) {
...
static void dumpToks(struct fileState * fs, struct tokenList * tokList, 
                     char * prefix) {
...
int getNextToks(struct fileState * fs, struct outputBuffer * ob, 
...
static int getNum(int * ptr, char * tok) {
...
static int getPort(int * ptr, char * tok) {
...
static int getProtocol(int * ptr, char * tok) {
...
static int getBoolean(int * ptr, char * tok) {

    int i;
    int rc;

    if ((rc = getNum(&i, tok))) return rc;
...
static int getList(char *** listPtr, char ** tokList) {
...
static int getHostname(struct in_addr * addr, char ** asName, char * tok) {
    struct hostent * host;
...
static void outputString(struct outputBuffer * ob, struct tokenList * toks, 
...
static void outputNum(struct outputBuffer * ob, struct tokenList * toks, 
...
static void outputIpAddr(struct outputBuffer * ob, struct tokenList * toks, 
...
static void outputList(struct outputBuffer * ob, struct tokenList * toks, 
...
static void outputActualServer(struct outputBuffer * ob,
                               struct lvsService * eservice, int saw) {
...
static int parseActualServer(struct fileState * fs, 
                             struct outputBuffer * ob,
                             struct lvsService * service,
                             struct lvsService * eservice) {
    ...
    while (*toks.argv && strcmp(toks.argv[0], "}")) {
        ...
        if (TOKEN("address")) {
            ...
            CHECK_ARGS(1) 
            if ((rc = getNum(&service->weight, toks.argv[2])))
                return rc;
...
static void outputVirtualServer(struct outputBuffer * ob, 
                                struct lvsVirtualServer * eserver,
                                int saw,
                                int outputRealServers) {
    int i;
    ...
    if (!(saw & LVS_SAW_VS_VIRTUALADDRESS))
        outputFormat(ob, (char *) "\taddress = %s %s\n", 
            inet_ntoa(eserver->virtualAddress), eserver->virtualDevice);
    ...
    if (!(saw & LVS_SAW_VS_SCHEDULER) && 
            eserver->scheduler != LVS_SCHED_WLC) {
...
*/
    ...
    OUTPUT_MISSING_NUM(LVS_SAW_VS_REENTRYTIME, eserver->reentryTime,
                       180, (char *) "\treentry = %d\n")

    OUTPUT_MISSING_IPADDR(LVS_SAW_VS_MASTERSERVER, 
                        eserver->masterServer, (char *) "\tmaster = %s\n")
...
static int parseVirtualServer(struct fileState * fs, 
                              struct outputBuffer * ob,
    ...
    if (eserver)
        if (eserver->numServers)
            sectionMarkers = calloc(eserver->numServers,
    ...
    while (*toks.argv && strcmp(toks.argv[0], "}")) {
        ...
        if (TOKEN("address")) {
            ...
            if (eserver) {
                ...
                outputFormat(ob, (char *) "%s%s%s%s%s%s%s%s",
                    toks.argv[0], toks.ws[0], toks.argv[1], toks.ws[1],
                    inet_ntoa(vserver->virtualAddress), toks.ws[2], 
                    vserver->virtualDevice, toks.ws[3]);
            }
            ...
            CHECK_ARGS(1)
            if (inet_aton(toks.argv[2], &vserver->vip_nmask)) {
            ...
            CHECK_ARGS(1)
            if (inet_aton(toks.argv[2], &vserver->pmask))
            ...
            if (eserver) {
                if (vserver->pmask.s_addr) {
                    ...
                    outputFormat(ob, (char *) "\t%s%s%s%s%s%s",
                        toks.argv[0], toks.ws[0], toks.argv[1], toks.ws[1],
                        inet_ntoa(vserver->pmask), toks.ws[2]);
                }
            }
            ...
            if (toks.quoteType[2]) {
                ...
                (void) sprintf(vserver->send_str, "%c%s%c",
                    toks.quoteType[2], toks.argv[2], toks.quoteType[2]);
            }
            else
        ...
        } else if (TOKEN("start_cmd")) {    /* Only on failover service */
            CHECK_ARGS(1)
        ...
        } else if (TOKEN("stop_cmd")) {     /* Only on failover service */
            CHECK_ARGS(1)
            ...
            CHECK_ARGS(1)
            if ((rc = getNum(&vserver->timeout, toks.argv[2])))
                return rc;
            ...
            CHECK_ARGS(1)
            if ((rc = getPort(&vserver->port, toks.argv[2])))
                return rc;
            ...
            CHECK_ARGS(1)
            if ((rc = getHostname(&vserver->masterServer, NULL, toks.argv[2]))) 
                return rc;
            OUTPUT_IPADDR(eserver, masterServer, 2, LVS_SAW_VS_MASTERSERVER);

        } else if (TOKEN("dirs")) {
            CHECK_EQUAL
            if ((rc = getList(&vserver->dirs, toks.argv + 2))) 
                return rc;
            if (eserver)
            ...
            if (!toks.argv[1] || !toks.argv[2]) return LVS_ERROR_MISSINGARGS;
            if (toks.argv[3]) return LVS_ERROR_EXTRAARGS;
            if (strcmp(toks.argv[2], "{")) return LVS_ERROR_MISSINGARGS;

            i = vserver->numServers++;
            vserver->servers = realloc(vserver->servers,
                    sizeof(*vserver->servers) * vserver->numServers);
...
static int parseConfiguration(  struct fileState * fs, 
                                struct outputBuffer * ob, 
                                struct lvsConfig * config,
                                struct lvsConfig * eConfig) {

    struct tokenList toks;
    int rc, i, j;
    /*** int hasEquals; not needed ***/
    char * sectionMarkers = NULL;
    char * current_ptr;
    int saw = 0;

    if (eConfig)
        sectionMarkers = calloc(eConfig->numVirtServers, 
            sizeof(*sectionMarkers));
    ...
    while (*toks.argv) {
        ...
        /*** hasEquals = toks.argv[1] ? !strcmp(toks.argv[1], "=") : 0; ***/
        ...
        if (TOKEN("primary")) {
            ...
            CHECK_ARGS(1)
            if (inet_aton(toks.argv[2], &config->nat_nmask)) {
            ...
            CHECK_ARGS(2)
            if ((rc = getHostname(&config->natRouter, NULL, toks.argv[2])))
                return rc;
            ...
            if (eConfig) {
                ...
                outputFormat(ob, (char *) "%s%s%s%s%s%s%s%s",
                    toks.argv[0], toks.ws[0], toks.argv[1], toks.ws[1],
                    inet_ntoa(eConfig->natRouter), toks.ws[2], 
                    eConfig->natRouterDevice, toks.ws[3]);
            }
            ...
            */        
            ...
            config->virtServers = realloc(config->virtServers,
                    sizeof(*config->virtServers) * config->numVirtServers);

            memset(config->virtServers + i, 0, sizeof(*config->virtServers));

            if (TOKEN("failover"))
                config->virtServers[i].failover_service = -1;

            config->virtServers[i].name          = strdup(toks.argv[1]);
            config->virtServers[i].protocol      = IPPROTO_TCP;
            config->virtServers[i].port          = 80;   /* http */
            config->virtServers[i].scheduler     = LVS_SCHED_WLC;
            config->virtServers[i].timeout       = 10;
            config->virtServers[i].reentryTime   = 180;
            config->virtServers[i].loadMonitor   = NULL;
            config->virtServers[i].persistent    = 0;
            config->virtServers[i].send_str      = NULL;
            config->virtServers[i].expect_str    = NULL;
            config->virtServers[i].start_cmd     = NULL;
            config->virtServers[i].stop_cmd      = NULL;
            config->virtServers[i].clientMonitor = 
                    strdup("/usr/sbin/nanny");

            if (eConfig) {
                for (j = 0; j < eConfig->numVirtServers; j++)
                    if (!strcmp(eConfig->virtServers[j].name,
                                config->virtServers[i].name)) break;
                if (j == eConfig->numVirtServers)
                    j = -1;
                else {
                    outputString(ob, &toks, 1, eConfig->virtServers[j].name);
                    if (sectionMarkers)
                        sectionMarkers[j] = 1;
                }
            } else {
            ...
            rc = parseVirtualServer(fs, ob, config->virtServers + i,
                j == -1 ? NULL : eConfig->virtServers + j);
    ...
    if (eConfig) {
        ...
        if (*((int *) &eConfig->natRouter) && !(saw & LVS_SAW_NATROUTER))
            outputFormat(ob, (char *) "nat_router = %s %s\n", 
                inet_ntoa(eConfig->natRouter), eConfig->natRouterDevice);
    	...
    	OUTPUT_MISSING_STR(LVS_SAW_DEBUG_LEVEL, eConfig->debug_level,
    	    (char *) "debug_level", (char *) "debug_level = %s\n")

        OUTPUT_MISSING_STR(LVS_SAW_VSADM, eConfig->vsadm,
            (char *) "/usr/sbin/ipvsadm",
            (char *) "ipvsadm = %s\n")
        ...
        OUTPUT_MISSING_NUM(LVS_SAW_DEADTIME, eConfig->deadTime,
            5, (char *) "deadtime = %d\n")

        OUTPUT_MISSING_NUM(LVS_SAW_USEFILESYNC, eConfig->useFileSync,
            0, (char *) "sync = %d\n")

        if (!(saw & LVS_SAW_REDIRECTTYPE) && 
                (eConfig->redirectType != LVS_REDIRECT_NAT)) {
...
int lvsParseConfig(int fd, struct lvsConfig * config, int * badLineNum) {
    ...
    config->vsadm               = strdup("/usr/sbin/ipvsadm");
    config->primaryServerName   = strdup("0.0.0.0");
    config->backupServerName    = strdup("0.0.0.0");
    config->redirectType        = LVS_REDIRECT_NAT;
